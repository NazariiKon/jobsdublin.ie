...
async def init_database_with_fake_data(session: AsyncSession = Depends(get_session)):
    ...
    user = await user_service.create_employer(UserCreate(authType="employer", email="admin@gmail.com", password="admin@gmail.com", name="Volodymyr", surname="Zelenskiy",
                                        picture="https://upload.wikimedia.org/wikipedia/commons/5/5c/Volodymyr_Zelenskyy_in_2022.jpg"))
    employer = await user_service.get_employer_by_user_id(user.id)
    employer = EmployerRead.model_validate(employer)
    print(employer, "ALLAH")
